...
def calculate_inception_stats(
    ...
    num_workers=1, prefetch_factor=2, device=torch.device('cuda'),
):
    # Rank 0 goes first.
    # if dist.get_rank() != 0:
    ...
    feature_dim = 2048
    with edm.dnnlib.util.open_url(detector_url) as f:
...
def calc(image_path, ref_path, num_expected, seed, batch):
    ...
    ref = None
    if dist.get_rank() == 0:
        with edm.dnnlib.util.open_url(ref_path) as f:
            ref = dict(np.load(f))
...
if __name__ == "__main__":
    ...
    print(torch.__version__)  # Ensure this matches the expected version
    print(torch.cuda.is_available())  # Should return True if CUDA is supported
