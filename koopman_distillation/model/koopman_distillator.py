...
class KoopmanDistillOneStepDMD(torch.nn.Module):
    def __init__(self, x0_observables_encoder, x_T_observables_encoder, x0_observables_decoder, rec_loss_type,
                 noisy_latent=0.2):
        super(KoopmanDistillOneStepDMD, self).__init__()
        self.x_0_observables_encoder = x0_observables_encoder
        self.x_T_observables_encoder = x_T_observables_encoder
        self.x0_observables_decoder = x0_observables_decoder
    ...
    def sample(self, batch_size, device, data_shape, sample_iter=1, data_batch=None):
        ...
        # the dynamical system start at state S in time 'T' in go backward in time to time 't'
        z_0 = self.x_0_observables_encoder(x_0, t)
        z_T_data = self.x_T_observables_encoder(x_T_data, T)

        K = torch.linalg.lstsq(z_T_data.reshape(z_0.shape[0], -1), z_0.reshape(z_0.shape[0], -1)).solution
        zT = self.x_T_observables_encoder(torch.randn_like(x_T_data), T)

        zt0_push = zT @ K
        xt0_push_hat = self.x0_observables_decoder(zt0_push, t)

        return xt0_push_hat, x_T_data